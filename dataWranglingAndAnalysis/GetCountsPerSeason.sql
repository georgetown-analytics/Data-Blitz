DELIMITER $$
CREATE DEFINER=`db_gtown_2018`@`%` PROCEDURE `GetCountsPerSeason`(IN In_ProBowl int)
BEGIN
SELECT 
	SEASON,
	Counts.IN_PROBOWL_TEAM AS IN_PROBOWL_TEAM, 
    COUNT(KICKER_ID) AS KICKER_COUNTS, 
    MIN(PLAY_COUNT) AS MIN_PLAY_COUNT,
...
ORDER BY PerPlay.SEASON , FKICKER
       ...
       WHERE Counts.IN_PROBOWL_TEAM=In_ProBowl
       GROUP BY Counts.SEASON;
